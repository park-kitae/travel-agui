...
@pytest.mark.asyncio
async def test_apply_client_state_unchanged_state_yields_no_event(manager):
    raw_state = {"travel_context": {"destination": "도쿄"}}
    first_events = [e async for e in manager.apply_client_state("thread-same", raw_state)]
    second_events = [e async for e in manager.apply_client_state("thread-same", raw_state)]
...
async def test_apply_tool_call_search_flights_one_way(manager):
    ...
    assert isinstance(events[0], StateDeltaEvent)
    state = manager.get("thread-13")
    assert state.travel_context.trip_type == "one_way"
...
@pytest.mark.asyncio
async def test_apply_tool_call_unchanged_state_yields_no_event(manager):
    args = {"city": "오사카", "check_in": "2026-07-01", "check_out": "2026-07-04", "guests": 3}
    first_events = [e async for e in manager.apply_tool_call("thread-repeat", "search_hotels", args)]
    second_events = [e async for e in manager.apply_tool_call("thread-repeat", "search_hotels", args)]
    assert len(first_events) == 1
    assert second_events == []
